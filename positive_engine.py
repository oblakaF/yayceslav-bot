...
SHOW_RESULT_RE = re.compile(
    r"\b(?:"
    r"зацени[,:]?\s+(?:мой|мою|мо[её]|что\s+я|вот\s+мой)|"
    r"оцени[,:]?\s+(?:мой|мою|мо[её])|"
    r"смотри\s+что\s+(?:я\s+)?сделал|"
    r"вот\s+что\s+(?:я\s+)?сделал|"
    r"вот\s+мой\s+(?:результат|проект|текст|дизайн|код)"
    r")\b",
    re.IGNORECASE,
)
