...
def _target_report_date(now):
    # Normal path: wait until the end of the last calendar day so almost the
    # entire month is counted. February automatically means 28/29.
    if runtime.is_last_calendar_day(now.date()) and (
        now.hour > 23 or (now.hour == 23 and now.minute >= 50)
    ):
        return now.date()

    # Catch-up path: if Railway was down at 23:50, the first scheduler tick on
