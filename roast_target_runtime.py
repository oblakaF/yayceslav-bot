...
priority order: reply target > explicit @mention/name > legacy fallback.
"""
...
async def enhanced_roast_command(update, context) -> None:
    """Honor an explicit target; otherwise preserve the existing /roast logic."""
...
def install(bot_module: Any | None = None) -> bool:
    ...
    logging.warning("Roast target runtime ready: reply > explicit target > legacy fallback")
    return True
