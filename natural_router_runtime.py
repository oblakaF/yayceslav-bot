...
_ROUTE_PATTERNS: tuple[tuple[str, tuple[re.Pattern[str], ...]], ...] = (
    ...
    (
        "roast",
        (
            re.compile(r"\bпрожарь(?:\s+(?:меня|себя|@?[\wёЁ][\wёЁ._-]*))?\b", re.I),
            re.compile(r"\bподколи\s+(?:меня|себя|его|её|ее|это|сообщение|@?[\wёЁ][\wёЁ._-]*)\b", re.I),
            re.compile(r"\bразъеби\s+(?:меня|себя|его|её|ее|@?[\wёЁ][\wёЁ._-]*)\b", re.I),
            re.compile(r"\bпрожарка\s+(?:для\s+)?(?:меня|себя|@?[\wёЁ][\wёЁ._-]*)\b", re.I),
            re.compile(r"\b(?:оскорби|отжарь|разнеси|обосри|размажь)\s+(?:меня|себя|его|её|ее|этого|эту|@?[\wёЁ][\wёЁ._-]*)\b", re.I),
            re.compile(r"\b(?:пройдись|проедься)\s+по\s+(?:мне|себе|нему|ней|этому|этой|@?[\wёЁ][\wёЁ._-]*)\b", re.I),
            re.compile(r"\b(?:дай|устрой)\s+(?:мне|себе|ему|ей|@?[\wёЁ][\wёЁ._-]*)\s+(?:прожарк\w*|разнос\w*)\b", re.I),
        ),
    ),
    (
        "debate",
