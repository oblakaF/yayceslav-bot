...
def test_sticker_tuning_is_modest_and_bounded(monkeypatch):
    ...
    assert sticker_engine.BACKGROUND_STICKER_CHANCE_CAP == 0.03
    assert sticker_interaction.QUESTION_STICKER_REPLY_CHANCE == 0.07
    assert sticker_post_runtime.POST_TEXT_TAG_CHANCE == 0.08
    assert max(sticker_engine.EVENT_CHANCE.values()) <= 0.03
