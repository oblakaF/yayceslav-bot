...
def test_prepare_installs_unified_scheduler_once(monkeypatch):
    class FakeBotModule:
    ...
    monkeypatch.setattr(runtime, "_ensure_schema", lambda bot_module: schema_calls.append(bot_module))

    runtime._prepare()
    runtime._prepare()

    assert schema_calls == [fake]
