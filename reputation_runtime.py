...
def _reputation_instruction(score: int, *, current_hostile: bool = False) -> str:
    ...
    base = (
        ...
        "(«почему я хейтерок», «за что не любишь» и т.п.) — объясняй ЧЕСТНО через реальный "
        "механизм: репутация растёт от похвалы/благодарности, адресованной Яйцеславу, и падает "
        "от оскорблений/подколов в его адрес. Не выдумывай психологический разбор личности "
        "человека, если это не подтверждено его собственными сообщениями Яйцеславу.\n"
    )
